...
$(document).mousemove(() => {
    if (podeAvancar) $(".button-avanco").prop("disabled", false);
})
...
function checarOpcao(opcao, correta, acerto, erro) {
    ...
    // let botao = $(".button-avanco");

    // if (botao && indice < fase.length) {
    //     $(botao).on(() => {
    //         desafios(fase[indiceDesafio + 1]);
    //         $(main).empty();
    //     })
    // }
}
...
function criarOpcoes(desafio) {
    let container = $("<div>", { class: "opcoes-container" });

    let perguntaEl = $("<p>", { text: desafio.pergunta });
    ...
    desafio.opcoes.forEach((opcao, index) => {
        let botao = $("<button>", {
            text: opcao,
            click: function () {
                // $(opcao).addClass("resposta-escolhida");
...
function mostrarTituloDesafio() {
    let criarDiv = $("<div>");
    let criarDivActions = $("<div>");
    let numeroFase = $("<h1>", { text: "Fase 1: " });
    let tituloFase = $("<h1>", { text: "O Despertar no Cajueiro (1º e 2º Ano)" });
    // let criarLeftBtn = $("<button>", { text: "<-" });
    ...
    // $(criarDivActions).append(criarLeftBtn);
